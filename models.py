...
class SocialClick(Base):
    """
    Track clicks on social media platforms from the social links page.
    """
    __tablename__ = "social_clicks"

    id = Column(Integer, primary_key=True, index=True)
    platform = Column(String(50), nullable=False, index=True)  # facebook, instagram, youtube, etc.
    ...
    # Session tracking (optional - to link with QR scan)
    session_id = Column(String(100), index=True)  # Can link to QR scan session
    user_agent = Column(Text)
